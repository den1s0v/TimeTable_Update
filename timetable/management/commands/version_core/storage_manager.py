...
class StorageManager(ABC):
    ...
    def dell_storages_by_file_version(self, file_version: FileVersion):
        ...
        for storage in storages:
            self.dell_file_by_storage(storage)
    ...
    def dell_file_by_storage(self, storage: Storage):
        ...
        if self._fs_root.exists(storage.path):
            try:
                ...
                logger.warning(f"Failed to delete file {storage.path}: {e}")
    ...
    def __make_file_version_is_archive(
        ...
        # КЛЮЧЕВАЯ ПРОВЕРКА: существует ли файл?
        if not self._fs_root.exists(current_path):
            logger.warning(
                f"Cannot archive missing file: {current_path} (resource: {resource.name})"
            )
            # Обновляем путь в БД на архивный (чтобы не пытаться снова)
            storage.path = archive_path
        ...
        try:
            ...
            logger.error(
                f"Failed to archive file {current_path} -> {archive_path}: {e}",
                exc_info=True,
            )
    ...
    def __rewrite_file(
        ...
        """
        try:
            fs_destination.makedirs(
                fs.path.dirname(destination_file_path), recreate=True
            )
            ...
            logger.error(
                f"Failed to rewrite file {destination_file_path}: {e}", exc_info=True
            )
            raise
    ...
    def __copy_file(
        ...
        try:
            ...
            logger.error(
                f"Failed to copy file {source_file_path} -> {destination_file_path}: {e}",
                exc_info=True,
            )
            raise
